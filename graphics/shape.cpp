                  ...
                  const std::vector<Eigen::Vector3f> &normals)
{
    ...
    std::vector<Eigen::Vector3f> normals1;
    ...
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(float) * verts.size() * 3, sizeof(float) * verts.size() * 3, static_cast<const void *>(normals1.data()));
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void Shape::init(const std::vector<Eigen::Vector3f> &vertices, const std::vector<Eigen::Vector3i> &triangles,
                 const std::vector<Eigen::Vector4i> &tetIndices, const std::vector<Eigen::Vector3f> &normals)
{
    init(vertices, triangles,normals);
...
void Shape::init(const std::vector<Eigen::Vector3f> &vertices, const std::vector<Eigen::Vector3f> &normals)
{
    if(vertices.size() != normals.size()) {
        std::cerr << "Vertices and normals are not the same size" << std::endl;
        return;
